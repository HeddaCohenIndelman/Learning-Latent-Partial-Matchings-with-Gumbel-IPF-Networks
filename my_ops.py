...
def row_mult(input, vector):
    ...
    return input * vector.view(-1, *extra_dims)

def IPF(log_alpha, row_marginals, column_marginals, epsilon = 1e-3, iters = 200):
    m = np.array(log_alpha.detach().cpu().numpy()).astype(float)
    xip = np.array(row_marginals.detach().cpu().numpy()).astype(float)
    xpj = np.array(column_marginals.detach().cpu().numpy()).astype(float)
    aggregates = [xip, xpj]
    dimensions = [[0], [1]]
...
def my_IPF(log_alpha, row_marginals, column_marginals, epsilon = 1e-8, iters = 25):
    """Performs incomplete IPF normalization to log_alpha.
        ...
        marginals: [[n_rows],[n_cols]] of n_row and n_column marginals
        Returns:
        A 3D tensor of close-to-proportinal matrices (2D tensors are
          converted to 3D tensors with batch_size equals to 1)
    """
    if torch.isnan(log_alpha).any():
        print("nans in ", log_alpha)

    #batch_size = log_alpha.size()[0]
    if log_alpha.ndim == 2:
        n_rows = log_alpha.size()[0]
    ...
    for i in range(iters):
        ...
        # normalize on rows
        log_alpha = log_alpha*row_marginals[:, None]/(log_alpha.sum(dim=2, keepdim=True)+epsilon)

        # normalize on columns
        log_alpha = log_alpha*column_marginals[None, :]/ (log_alpha.sum(dim=1, keepdim=True)+epsilon)

    # exponentiate
    #log_alpha = torch.exp(log_alpha)
    if torch.isnan(log_alpha).any():
        print("after ipfp", log_alpha)
    return log_alpha
