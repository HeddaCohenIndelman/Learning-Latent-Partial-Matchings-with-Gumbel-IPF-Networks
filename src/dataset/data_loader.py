...
class GMDataset(Dataset):
        ...
        try:
            if cfg.MATCHING_TYPE == 'Balanced':
                ...
                if cfg.filter_type == 'NoFilter':
                    ...
                    anno_pair, perm_mat = self.ds.get_pair_unbalanced(cls)
            #anno_pair, perm_mat = self.ds.get_pair(cls, tgt_outlier=cfg.PROBLEM.TGT_OUTLIER, src_outlier=cfg.PROBLEM.SRC_OUTLIER)
        except TypeError:
    ...
    def get_multi(self, idx, cls):
        ...
        elif cfg.MATCHING_TYPE == 'Unbalanced' and cfg.filter_type == 'NoFilter':
            anno_list, perm_mat_list = self.ds.get_multi(cls, num=num_graphs, filter_outlier=False)
